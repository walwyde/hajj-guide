...
// import DuaPlayer from "@/components/duaPlayer"

// DuaPlayer Component
function DuaPlayer({ audioSrc, title }: { audioSrc: string; title: string }) {
  ...
  useEffect(() => {
    const audioElement = new Audio(`/${audioSrc}.mp3`)
...
export default function StepDetail() {
  ...
  const [step, setStep] = useState<typeof Step | null>(null) 
  ...
  useEffect(() => {
    const fetchStep = async () => {
      try {
        ...
        toast.error("Error loading step");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSectionComplete = (section: string) => {
    setSectionProgress((prev) => {
      ...
      const completed = Object.keys(updated).length >= 6 && Object.values(updated).every(Boolean)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br mt-4 from-background via-background to-muted/20">
      <ReadingProgress />
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <Card className="border-card-border hover:border-accent/30 transition-colors bg-gradient-to-br from-card to-secondary/5">
              ...
              {step.title.toLowerCase().includes("tawaf") && (
                <div className="px-6 pb-6">
                  <Button onClick={() => navigate.push("/tawaf-counter")} className="bg-purple-400">
                    Use Tawaf Counter
                  </Button>
                </div>
              )}
            ...
            {/* Dua Section */}
            <Card className="border-card-border hover:border-accent/30 transition-colors bg-gradient-to-br from-card to-green-500/5 border-green-200">
              <CardHeader>
              ...
              <CardContent className="space-y-6">
                {/* Arabic Text */}
                <div className="text-right text-2xl font-arabic leading-relaxed p-4 bg-background/50 rounded-lg border-r-4 border-green-500">
                  {step.dua.arabicText}
                </div>
                
                {/* Audio Player */}
                <div className="bg-secondary/20 p-4 rounded-lg">
                  <h4 className="font-medium mb-3 text-sm text-muted-foreground">Listen to the Dua:</h4>
                  <DuaPlayer  audioSrc={step.dua.audioSource} title={step.title} />
                </div>
                
                {/* Animated Content */}
                <AnimatedCanvasText
                  text={`Transliteration: ${step.dua.transliteration}\n\nTranslation: "${step.dua.translation}"`}
                  speed={35}
                  onComplete={() => handleSectionComplete("dua")}
                />
              </CardContent>
            </Card>
          </div>
