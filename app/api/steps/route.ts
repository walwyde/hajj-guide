...
export async function GET(req: Request) {
  try {
    ...
    // Update lastSeen asynchronously (no need to block response)
    User.findByIdAndUpdate(session.user.id, { lastSeen: new Date() }).catch(console.error);

    // Return user progress + steps
...
export async function PATCH(req: Request) {
  try {
    ...
    // Update if step exists, otherwise push a new one
    ...
    if (updateResult.matchedCount === 0) {
      // Add new step if it didn't exist
      await User.updateOne(
        { _id: session.user.id },
        {
          $push: {
    ...
    console.error("PATCH /api/steps error:", error);
    return NextResponse.json({ error: "Server error" }, { status: 500 });
  }
}
